function drawScatterPlotWithTrend(df, id, region, x, y, r, c){
  ...
  df = df.filter(function(d, i){ 
  	return (d.region == region || region == 'World') && (d.score != 0 && d.score_old != 0); 
  });
  ...
  svg.append("g").selectAll().data(df).enter().append("circle")
	  .attr("cx", function(d,i){return xScale(d[y] || 0);})
	  .attr("cy", function(d,i){return yScale(d[y] || 0);})
	  .attr("r", function(d,i){return rScale(Math.abs(10));})
	  .attr("fill", function(d){ return "darkgray"; })
